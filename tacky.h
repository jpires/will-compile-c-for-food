...
using instruction = std::variant<return_statement,
                                 ...
                                 uint_to_double,
                                 zero_extend>;
struct plain_operand
{
    val dst;
};
struct dereferenced_pointer
{
    val dst;
};
using exp_result = std::variant<plain_operand, dereferenced_pointer>;
...
std::string pretty_print(const get_address &i, int32_t ident = 0);
...
std::string pretty_print(const load &i, int32_t ident = 0);
...
std::string pretty_print(const store &i, int32_t ident = 0);
